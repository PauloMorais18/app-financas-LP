...
const product = (r:any) => ({id:r.id,userId:r.user_id,groupId:r.group_id,name:r.name,costPerMeter:Number(r.cost_per_meter),filamentMeters:Number(r.filament_meters),saleValue:Number(r.sale_value),totalCost:Number(r.total_cost),active:r.active,createdAt:r.created_at,updatedAt:r.updated_at});
...
const order = (r: any) => ({ id:r.id,userId:r.user_id,groupId:r.group_id,sourceId:r.source_id,colorId:r.color_id||"",colorName:r.colors?.name||"",clientId:r.client_id||"",productId:r.product_id||"",title:r.title,customer:r.customer,dueDate:dateOnly(r.due_date),value:Number(r.value),status:r.status,observation:r.observation,createdAt:r.created_at,updatedAt:r.updated_at });
const transactionRow = (v:any) => ({ user_id:v.userId,group_id:v.groupId,source_id:v.sourceId||null,client_id:v.clientId||null,product_id:v.productId||null,date:v.date,description:v.description,category:v.category,type:v.type,value:v.value,payment_method:v.paymentMethod,status:v.status,observation:v.observation||"",recurring:Boolean(v.recurring) });
const orderRow = (v:any) => ({ user_id:v.userId,group_id:v.groupId,source_id:v.sourceId,color_id:v.colorId||null,client_id:v.clientId||null,product_id:v.productId||null,title:v.title,customer:v.customer,due_date:v.dueDate,value:v.value,status:v.status,observation:v.observation||"" });
...
async function get<T>(raw:string):Promise<Response<T>>{const u=url(raw),p=u.pathname,params=u.searchParams;
  ...
  if(p==="/groups"){const user=await currentUser();const{data:members,error:me}=await supabase.from("group_members").select("group_id").eq("user_id",user.id);fail(me);const ids=(members||[]).map((m:any)=>m.group_id);if(!ids.length)return{data:[] as T};const{data,error}=await supabase.from("groups").select("*").in("id",ids).order("created_at");fail(error);return{data:(data||[]).map((g:any)=>({id:g.id,name:g.name,code:g.code,ownerId:g.owner_id,isDefault:g.is_default,memberCount:g.member_count||1,createdAt:g.created_at})) as T}}
  if(p==="/income-sources"){await currentUser();let q:any=supabase.from("income_sources").select("*").order("name");if(params.get("userId"))q=q.eq("user_id",params.get("userId"));if(params.get("groupId"))q=q.eq("group_id",params.get("groupId"));const{data,error}=await q;fail(error);return{data:(data||[]).map(source) as T}}
  if(p==="/companies"){await currentUser();let q:any=supabase.from("companies").select("*").order("name");if(params.get("userId"))q=q.eq("user_id",params.get("userId"));if(params.get("groupId"))q=q.eq("group_id",params.get("groupId"));const{data,error}=await q;fail(error);return{data:(data||[]).map(company) as T}}
  if(p==="/colors"){await currentUser();let q:any=supabase.from("colors").select("*").order("name").limit(5);if(params.get("userId"))q=q.eq("user_id",params.get("userId"));if(params.get("groupId"))q=q.eq("group_id",params.get("groupId"));if(params.get("search"))q=q.ilike("name",`%${params.get("search")}%`);const{data,error}=await q;fail(error);return{data:(data||[]).map(color) as T}}
  if(p==="/clients"||p==="/products"){await currentUser();const table=p==="/clients"?"clients":"products",mapper=p==="/clients"?client:product;let q:any=supabase.from(table).select("*").order("name");if(params.get("groupId"))q=q.eq("group_id",params.get("groupId"));if(params.get("search"))q=q.ilike("name",`%${params.get("search")}%`);const{data,error}=await q;fail(error);return{data:(data||[]).map(mapper) as T}}
  ...
  if(p==="/orders"){await currentUser();let q:any=supabase.from("orders").select("*,colors(name)").order("due_date");if(params.get("userId"))q=q.eq("user_id",params.get("userId"));if(params.get("groupId"))q=q.eq("group_id",params.get("groupId"));if(params.get("sourceId"))q=q.eq("source_id",params.get("sourceId"));const{data,error}=await q;fail(error);return{data:(data||[]).map(order) as T}}
...
async function post<T>(p:string,v?:any):Promise<Response<T>>{
  ...
  else if(p==="/products")result=await supabase.from("products").insert({user_id:user.id,group_id:v.groupId,name:v.name,cost_per_meter:v.costPerMeter||0,filament_meters:v.filamentMeters||0,sale_value:v.saleValue||0}).select().single();
...
async function put<T>(p:string,v:any):Promise<Response<T>>{const user=await currentUser(),id=p.split("/").pop();let result:any,mapper:any;
  ...
  else if(p.startsWith("/products/")){result=await supabase.from("products").update({name:v.name,cost_per_meter:v.costPerMeter||0,filament_meters:v.filamentMeters||0,sale_value:v.saleValue||0,active:v.active}).eq("id",id).select().single();mapper=product}
