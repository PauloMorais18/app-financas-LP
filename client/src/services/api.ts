...
const source = (r: any) => ({ id:r.id,userId:r.user_id,groupId:r.group_id,name:r.name,description:r.description,active:r.active,createdAt:r.created_at,updatedAt:r.updated_at });
const company = (r: any) => ({ id:r.id,userId:r.user_id,groupId:r.group_id,name:r.name,active:r.active,createdAt:r.created_at,updatedAt:r.updated_at });
const color = (r: any) => ({ id:r.id,userId:r.user_id,groupId:r.group_id,name:r.name,createdAt:r.created_at,updatedAt:r.updated_at });
const client = (r:any) => ({id:r.id,userId:r.user_id,groupId:r.group_id,name:r.name,place:r.place||"",phone:r.phone||"",active:r.active,createdAt:r.created_at,updatedAt:r.updated_at});
...
const saveOrderProducts=async(orderId:string,items:any[],desiredTotal:number)=>{
  if(!items.length)return;
  const rows=items.map((item:any)=>({order_id:orderId,product_id:item.productId,quantity:Math.max(1,Number(item.quantity)||1),unit_sale_value:Math.max(0,Number(item.saleValue)||0)}));
  let {error}=await supabase.from("order_products").insert(rows);
  if(error&&(error.code==="PGRST204"||error.code==="42703")){
    ({error}=await supabase.from("order_products").insert(rows.map(({unit_sale_value,...row}:any)=>row)));
    if(!error){const fallback=await supabase.from("orders").update({value:Math.max(0,Number(desiredTotal)||0)}).eq("id",orderId);error=fallback.error}
  }
  fail(error);
};
...
async function get<T>(raw:string):Promise<Response<T>>{const u=url(raw),p=u.pathname,params=u.searchParams;
  ...
  if(p==="/users"){const user=await currentUser();if(!params.get("groupId")){const{data,error}=await supabase.from("profiles").select("*").eq("id",user.id);fail(error);return{data:(data||[]).map(profile) as T}}const{data:members,error:memberError}=await supabase.from("group_members").select("user_id").eq("group_id",params.get("groupId"));fail(memberError);const ids=(members||[]).map((member:any)=>member.user_id);if(!ids.length)return{data:[] as T};const{data,error}=await supabase.from("profiles").select("*").in("id",ids).order("name");fail(error);return{data:(data||[]).map(profile) as T}}
  if(p==="/groups"){const user=await currentUser();const{data:members,error:me}=await supabase.from("group_members").select("group_id").eq("user_id",user.id);fail(me);const ids=(members||[]).map((m:any)=>m.group_id);if(!ids.length)return{data:[] as T};const{data,error}=await supabase.from("groups").select("*").in("id",ids).order("created_at");fail(error);return{data:(data||[]).map((g:any)=>({id:g.id,name:g.name,code:g.code,ownerId:g.owner_id,isDefault:g.is_default,memberCount:g.member_count||1,filamentCostPerMeter:Number(g.filament_cost_per_meter??0.3),workshopContracted:Boolean(g.workshop_contracted),workshopEnabled:Boolean(g.workshop_contracted&&g.workshop_enabled),createdAt:g.created_at})) as T}}
  if(p==="/income-sources"){await currentUser();if(!params.get("groupId"))return{data:[] as T};let q:any=supabase.from("income_sources").select("*").order("name");if(params.get("userId"))q=q.eq("user_id",params.get("userId"));if(params.get("groupId"))q=q.eq("group_id",params.get("groupId"));const{data,error}=await q;fail(error);return{data:(data||[]).map(source) as T}}
  if(p==="/companies"){await currentUser();if(!params.get("groupId"))return{data:[] as T};let q:any=supabase.from("companies").select("*").order("name");if(params.get("userId"))q=q.eq("user_id",params.get("userId"));if(params.get("groupId"))q=q.eq("group_id",params.get("groupId"));const{data,error}=await q;fail(error);return{data:(data||[]).map(company) as T}}
  if(p==="/colors"){await currentUser();if(!params.get("groupId"))return{data:[] as T};let q:any=supabase.from("colors").select("*").order("name").limit(5);if(params.get("userId"))q=q.eq("user_id",params.get("userId"));if(params.get("groupId"))q=q.eq("group_id",params.get("groupId"));if(params.get("search"))q=q.ilike("name",`%${params.get("search")}%`);const{data,error}=await q;fail(error);return{data:(data||[]).map(color) as T}}
  if(p==="/clients"||p==="/products"){const authUser=await currentUser();if(!params.get("groupId"))return{data:[] as T};const table=p==="/clients"?"clients":"products",mapper=p==="/clients"?client:product;let q:any=supabase.from(table).select("*").order("name");if(params.get("groupId"))q=q.eq("group_id",params.get("groupId"));if(params.get("userId"))q=q.eq("user_id",params.get("userId"));if(params.get("search"))q=q.ilike("name",`%${params.get("search")}%`);const{data,error}=await q;fail(error);if(p==="/products")console.info("[Finanbase][produtos-db]",{authenticatedUserId:authUser.id,requestedGroupId:params.get("groupId"),requestedUserId:params.get("userId")||null,count:(data||[]).length,rows:(data||[]).map((row:any)=>({id:row.id,name:row.name,groupId:row.group_id,userId:row.user_id}))});return{data:(data||[]).map(mapper) as T}}
  if(p.startsWith("/orders/")){await currentUser();const{data,error}=await supabase.from("orders").select(orderSelect).eq("id",p.split("/").pop()).single();fail(error);return{data:order(data) as T}}
  if(p==="/orders"){await currentUser();if(!params.get("groupId"))return{data:[] as T};let q:any=supabase.from("orders").select(orderSelect).order("created_at",{ascending:true});if(params.get("userId"))q=q.eq("user_id",params.get("userId"));if(params.get("groupId"))q=q.eq("group_id",params.get("groupId"));if(params.get("sourceId"))q=q.eq("source_id",params.get("sourceId"));const{data,error}=await q;fail(error);return{data:(data||[]).map(order) as T}}
...
async function post<T>(p:string,v?:any):Promise<Response<T>>{
  ...
  if(p==="/groups"){const code=crypto.randomUUID().replace(/-/g,"").slice(0,8).toUpperCase();const{data:g,error}=await supabase.from("groups").insert({owner_id:user.id,name:v.name,code}).select().single();fail(error);const{error:memberError}=await supabase.from("group_members").insert({group_id:g.id,user_id:user.id});fail(memberError);return{data:{id:g.id,name:g.name,code:g.code,ownerId:g.owner_id,isDefault:g.is_default,memberCount:1,createdAt:g.created_at} as T}}
  if(p==="/groups/join"){const{data,error}=await supabase.rpc("join_group_by_code",{group_code:String(v.code).trim().toUpperCase()});fail(error);const g=Array.isArray(data)?data[0]:data;return{data:{id:g.id,name:g.name,code:g.code,ownerId:g.owner_id,isDefault:g.is_default,memberCount:g.member_count||1,createdAt:g.created_at} as T}}
  if(p==="/income-sources")result=await supabase.from("income_sources").insert({user_id:user.id,group_id:v.groupId,name:v.name,description:v.description||""}).select().single();
  else if(p==="/companies")result=await supabase.from("companies").insert({user_id:user.id,group_id:v.groupId,name:v.name}).select().single();
  else if(p==="/colors")result=await supabase.from("colors").insert({user_id:user.id,group_id:v.groupId,name:v.name}).select().single();
  else if(p==="/clients")result=await supabase.from("clients").insert({user_id:user.id,group_id:v.groupId,name:v.name,place:v.place||"",phone:v.phone||""}).select().single();
  else if(p==="/products")result=await supabase.from("products").insert({user_id:user.id,group_id:v.groupId,name:v.name,cost_per_meter:v.costPerMeter||0,filament_meters:v.filamentMeters||0,printing_time_minutes:Math.max(0,Math.round(Number(v.printingTimeMinutes)||0)),sale_value:v.saleValue||0,image_url:v.imageUrl||"",model_file_url:v.modelFileUrl||""}).select().single();
  else if(p==="/transactions")result=await supabase.from("transactions").insert({...transactionRow(v),user_id:user.id}).select().single();
  else if(p==="/orders"){const{data:created,error:createError}=await supabase.from("orders").insert({...orderRow(v),user_id:user.id}).select("id").single();fail(createError);if(!created)throw new ApiError("Pedido não foi criado.");const productItems:any[]=(v.productItems||[]).filter((item:any)=>item.productId);await saveOrderProducts(created.id,productItems,v.value);const{data,error}=await supabase.from("orders").select(orderSelect).eq("id",created.id).single();fail(error);return{data:order(data) as T}}
  ...
  else throw new ApiError("Operação não suportada.",404);fail(result.error);const mapper=p==="/income-sources"?source:p==="/companies"?company:p==="/colors"?color:p==="/clients"?client:p==="/products"?product:p==="/transactions"?transaction:order;return{data:mapper(result.data) as T};
}
async function put<T>(p:string,v:any):Promise<Response<T>>{const user=await currentUser(),id=p.split("/").pop();let result:any,mapper:any;
  ...
  else if(p.startsWith("/groups/")){const changes:any={};if(v.name!==undefined)changes.name=String(v.name).trim();if(v.filamentCostPerMeter!==undefined)changes.filament_cost_per_meter=Number(v.filamentCostPerMeter);if(v.workshopEnabled!==undefined)changes.workshop_enabled=Boolean(v.workshopEnabled);result=await supabase.from("groups").update(changes).eq("id",id).eq("owner_id",user.id).select().single();mapper=(g:any)=>({id:g.id,name:g.name,code:g.code,ownerId:g.owner_id,isDefault:g.is_default,memberCount:g.member_count||1,filamentCostPerMeter:Number(g.filament_cost_per_meter??0.3),workshopContracted:Boolean(g.workshop_contracted),workshopEnabled:Boolean(g.workshop_contracted&&g.workshop_enabled),createdAt:g.created_at})}
  ...
  else if(p.startsWith("/clients/")){result=await supabase.from("clients").update({name:v.name,place:v.place||"",phone:v.phone||"",active:v.active}).eq("id",id).select().single();mapper=client}
  else if(p.startsWith("/products/")){result=await supabase.from("products").update({name:v.name,cost_per_meter:v.costPerMeter||0,filament_meters:v.filamentMeters||0,printing_time_minutes:Math.max(0,Math.round(Number(v.printingTimeMinutes)||0)),sale_value:v.saleValue||0,image_url:v.imageUrl||"",model_file_url:v.modelFileUrl||"",active:v.active}).eq("id",id).select().single();mapper=product}
  ...
  else if(p.startsWith("/orders/")){const orderId=String(id);const{error:updateError}=await supabase.from("orders").update(orderRow(v)).eq("id",orderId);fail(updateError);const{error:deleteItemsError}=await supabase.from("order_products").delete().eq("order_id",orderId);fail(deleteItemsError);const productItems:any[]=(v.productItems||[]).filter((item:any)=>item.productId);await saveOrderProducts(orderId,productItems,v.value);result=await supabase.from("orders").select(orderSelect).eq("id",orderId).single();mapper=order}
  else throw new ApiError("Operação não suportada.",404);fail(result.error);return{data:mapper(result.data) as T};
}
async function del<T>(p:string):Promise<Response<T>>{await currentUser();const id=p.split("/").pop();if(p.startsWith("/groups/")){const{error}=await supabase.rpc("delete_owned_group",{target_group:id});fail(error);return{data:undefined as T}}const table=p.startsWith("/income-sources/")?"income_sources":p.startsWith("/companies/")?"companies":p.startsWith("/clients/")?"clients":p.startsWith("/products/")?"products":p.startsWith("/colors/")?"colors":p.startsWith("/transactions/")?"transactions":p.startsWith("/orders/")?"orders":"";if(!table)throw new ApiError("Operação não suportada.",404);const{error}=await supabase.from(table).delete().eq("id",id);fail(error);return{data:undefined as T}}
