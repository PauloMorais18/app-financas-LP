...
function CatalogManagement({kind,notify}:{kind:"clients"|"products";notify:(message:string,error?:boolean)=>void}) {
  ...
  const visibleCatalogRecords=useMemo(()=>records.data
    ...
    .sort((a,b)=>a.name.localeCompare(b.name,"pt-BR",{sensitivity:"base"})),[records.data,search]);
  ...
  const [costPerMeter,setCostPerMeter]=useState(()=>activeGroup?.filamentCostPerMeter??0.3),[filamentMeters,setFilamentMeters]=useState(0),[printingTimeMinutes,setPrintingTimeMinutes]=useState(0),[saleValue,setSaleValue]=useState(0);
  ...
  const clear=()=>{setEditing(null);setName("");setPlace("");setPhone("");setCostPerMeter(activeGroup?.filamentCostPerMeter??0.3);setFilamentMeters(0);setPrintingTimeMinutes(0);setSaleValue(0);setImageFile(null);setImageUrl("");setModelFileUrl("")};
  ...
  return <><PageHeading title={`Cadastros · ${isClient?"Clientes":"Produtos"}`} subtitle={isClient?"Nome é obrigatório; local e telefone são opcionais.":"O custo total é calculado automaticamente."}/>
    <section className="management-grid"><article className="card form-card"><CardTitle title={editing?"Editar cadastro":"Novo cadastro"} subtitle="Preencha as informações abaixo."/><form onSubmit={submit}><div className="form-grid">
      ...
      {isClient?<><Field label="Lugar (opcional)"><input value={place} onChange={e=>setPlace(e.target.value)} placeholder="Cidade, bairro ou endereço"/></Field><Field label="Telefone (opcional)"><input value={phone} onChange={e=>setPhone(e.target.value)} placeholder="(00) 00000-0000"/></Field></>:<>
        ...
        <Field label="Tempo de impressão"><div className="duration-input"><label><input type="number" min="0" step="1" value={Math.floor(printingTimeMinutes/60)||""} onChange={e=>setPrintingTimeMinutes(Math.max(0,Number(e.target.value)||0)*60+printingTimeMinutes%60)}/><span>horas</span></label><label><input type="number" min="0" max="59" step="1" value={printingTimeMinutes%60||""} onChange={e=>setPrintingTimeMinutes(Math.floor(printingTimeMinutes/60)*60+Math.min(59,Math.max(0,Number(e.target.value)||0)))}/><span>min</span></label></div></Field>
    ...
    <article className="card"><CardTitle title={isClient?"Clientes cadastrados":"Produtos cadastrados"} subtitle={`${records.data.length} registro(s)`}/><div className="catalog-search"><input value={search} onChange={e=>setSearch(e.target.value)} placeholder={`Buscar ${isClient?"cliente":"produto"} por nome...`}/></div><div className="simple-list">{visibleCatalogRecords.map(record=><div className={`list-row ${"imageUrl" in record?"product-list-row":""}`} key={record.id}>{"imageUrl" in record&&(record.imageUrl?<img className="product-thumbnail" src={record.imageUrl} alt={record.name}/>:<span className="product-thumbnail placeholder"><ImageIcon/></span>)}<div><b>{record.name}</b><small>{"place" in record?[record.place,record.phone].filter(Boolean).join(" · ")||"Somente nome":`Custo ${money.format(record.totalCost)} · Venda ${money.format(record.saleValue)} · Impressão ${record.printingTimeMinutes?`${Math.floor(record.printingTimeMinutes/60)}h${record.printingTimeMinutes%60?` ${record.printingTimeMinutes%60}min`:""}`:"não informada"}`}</small>{"modelFileUrl" in record&&record.modelFileUrl&&<a className="model-file-link" href={record.modelFileUrl} target="_blank" rel="noreferrer"><ExternalLink/>Abrir arquivo 3MF/STL</a>}</div><div className="row-actions"><button onClick={()=>edit(record)}><Pencil/></button><button onClick={()=>remove(record.id)}><Trash2/></button></div></div>)}{!records.loading&&!visibleCatalogRecords.length&&<Empty text={search?"Nenhum cadastro encontrado nesta busca.":"Nenhum cadastro encontrado."}/>}</div></article></section></>;
}
